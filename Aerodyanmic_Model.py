...
class AerodynamicsModel:
    ...
    def FM_aero(self,Vt,alpha,beta,p,q,r,del_e,del_a,del_r,rho):
        ...
        self.CL = (self.parameters.CL0) + (self.parameters.CL_alpha*alpha) + (self.parameters.CL_q*self.q_hat) + (self.parameters.CL_de*del_e)
        ...
        self.Cn = (self.parameters.Cn_beta*beta) + (self.parameters.Cn_p*self.p_hat) + (self.parameters.Cn_r*self.r_hat) + (self.parameters.Cn_da*del_a) + (self.parameters.Cn_dr*del_r)
        ...
        self.Fx_aero = -self.D_aeroF * np.cos(alpha)*np.cos(beta) - self.Y_aerof * np.cos(alpha)*np.sin(beta) + self.L_aeroF * np.sin(alpha)
        self.Fy_aero = -self.D_aeroF * np.sin(beta) + self.Y_aerof * np.cos(beta)
        self.Fz_aero = -self.D_aeroF * np.sin(alpha)*np.cos(beta) - self.Y_aerof*np.sin(alpha)*np.sin(beta) - self.L_aeroF * np.cos(alpha)
